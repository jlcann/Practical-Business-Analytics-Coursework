...
determineThreshold<-function(testPredicted, expectedTestOutput){
  ...
  plot(thresholds$x,thresholds$tpr,
  ...
  lines(thresholds$x,100-thresholds$fpr,type="l",col="red",lwd=3,lty=1)
  
  # The point where specificity and sensitivity are the same
  crosspoint<-thresholds$x[which(thresholds$tpr<(100-thresholds$fpr))[1]]
  ...
  lines(thresholds$x,thresholds$distance,type="l",col="green",lwd=2,lty=3)
